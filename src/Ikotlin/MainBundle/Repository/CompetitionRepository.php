...
class CompetitionRepository extends EntityRepository {
    ...
    public function getCompetitions($starts_at, $length, $level, $order) {
        ...
        $qb
                ->select('c.id', 'c.title', 'c.created', 'c.solved', 'c.level', 'u.id as user_id', 'u.username as user_name', 'u.picture as user_picture')
                ->from('IkotlinMainBundle:Competition', 'c')
                ->leftJoin(
                        'IKotlin\MainBundle\Entity\User', 'u', \Doctrine\ORM\Query\Expr\Join::WITH, 'c.idUser = u.id'
                )
                ->where("c.level = :level")
    ...
    public function getCompetitionOptimized($idcomp) {
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb
                ->select('c.id', 'c.title', 'c.created', 'c.solved', 'c.level', 'c.content', 'u.id as user_id', 'u.username as user_name', 'u.picture as user_picture')
