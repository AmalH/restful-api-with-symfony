...
class ForumQuestionRepository extends EntityRepository {
    ...
    public function getForumsBy($start, $length, $searchkey, $orderby) {
        ...
        if (empty($searchQuery)) {
            $qb
                    ->select('f.id', 'f.subject', 'f.created', 'f.rating', 'f.tags', 'f.views', 'f.edited as edited', 'u.id as user_id', 'u.username as user_name', 'u.picture as user_picture')
                    ->from('IKotlin\MainBundle\Entity\ForumQuestion', 'f')
                    ->leftJoin(
                            'IKotlin\MainBundle\Entity\User', 'u', \Doctrine\ORM\Query\Expr\Join::WITH, 'f.idUser = u.id'
                    )
    ...
    public function getForumOptimized($forumid) {
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb
                ->select('f.id', 'f.subject', 'f.content', 'f.created', 'f.rating', 'f.tags', 'f.views', 'f.code as code', 'f.edited as edited', 'u.id as user_id', 'u.username as user_name', 'u.picture as user_picture')
                ->from('IKotlin\MainBundle\Entity\ForumQuestion', 'f')
                ->leftJoin(
                        'IKotlin\MainBundle\Entity\User', 'u', \Doctrine\ORM\Query\Expr\Join::WITH, 'f.idUser = u.id'
                )
